...
class Watcher(
  ...
  system.scheduler.schedule(1 day, 1 day, watcherRef, CleanRequest)(executor, watcherRef)
...
private class WatcherActor(
  ...
  def receiveCommand: Receive = {
    case WatchRequest =>
      ...
      for (r <- analyzer.process) {
        if (r.nonEmpty) self ! AddCommand(r)
        if (self != s) s ! WatchResponse
      }
    case CleanRequest =>
      val s = sender() // only inside actor
      for (r <- analyzer.findOld(state.records)) {
        if (r.nonEmpty) self ! RemoveCommand(r)
